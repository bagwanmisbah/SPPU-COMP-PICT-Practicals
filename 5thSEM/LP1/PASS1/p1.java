...
public class pass {
    ...
    public static void main(String args[]) throws Exception {
        ...
        File poolFile = new File("pooltable.txt");
        BufferedReader reader = new BufferedReader(new FileReader(file));
        BufferedWriter writer = new BufferedWriter(new FileWriter(file1));
        BufferedWriter poolWriter = new BufferedWriter(new FileWriter(poolFile));
        ...
        int k = 0;
        String buffer = "";
        pooltab[0] = 0; // Initialize the first pool tab entry
                ...
                if (y.equals("START")) {
                    ...
                    if (searchis(y)) {
                        ...
                        if (y.equals("LTORG")) {
                            ...
                            pooltab[poolcounter++] = litcounter;
                        ...
                        if (x[0] == '=') {
                            ...
                            if (!searchsym(y) && isflag == 0 && stp == 0) {
                                ...
                                if (splitted.hasMoreTokens())
                                    ans = "";
                            } else if (!searchsym(y) && isflag == 1 && stp == 0) {// if instruction has passed on the
                                                                                  // line then only add the symbol not
                                                                                  // the address
                                sym[symcounter++][0] = y;
                                ans += "(S," + Integer.toString(indexsym(y) + 1) + ")";
        ...
        for (int i = 0; i < poolcounter; i++) {
            ...
            poolWriter.write(pooltab[i] + "\n"); // Write to pool table file
        }
